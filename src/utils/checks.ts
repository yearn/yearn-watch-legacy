...
const checks = [
    {
        field: 'governance',
        validate: (value: any): boolean => typeof value === 'string' && value.toLowerCase() === GOVERNANCE.toLowerCase(),
        error: 'value incorrect for governance'
    },
    {
        field: 'guardian',
        validate: (value: any): boolean => typeof value === 'string' && value.toLowerCase() === GUARDIAN.toLowerCase(),
        error: 'value incorrect for guardian'
    },
    {
        field: 'management',
        validate: (value: any): boolean => typeof value === 'string' && value.toLowerCase() === MANAGEMENT.toLowerCase(),
        error: 'value incorrect for management'
    },
    {
        field: 'managementFee',
        validate: (value: any): boolean => typeof value === 'number' && value === MANAGEMENT_FEE,
        error: 'value incorrect for management fee'
    },
    {
        field: 'performanceFee',
        validate: (value: any): boolean => typeof value === 'number' && value === PERF_FEE,
...
export const vaultChecks = (vault: Vault): Vault => {
    ...
    checks.forEach(check => {
        ...
        if (!check.validate(vault[check.field])) {
            ...
            result.errors = result.errors ? [...result.errors, check.error] : [check.error];
        }
    })
