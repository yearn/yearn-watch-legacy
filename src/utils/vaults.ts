...
export const getVaults = async (): Promise<Vault[]> => {
    ...
    try {
      ...
      payload.forEach(vault => { 
          ...
          vault.strategies.forEach(strat => strategyMap.set(strat.address, vault.address));
      });

      const vaultCalls: ContractCallContext[] = payload.map(({ address }) => {
        const calls = VAULT_VIEW_METHODS.map(method => ({ reference: method, methodName: method, methodParameters: [] }));
        return {
                reference: address,
      ...
      const stratCalls: ContractCallContext[] = payload.flatMap(({ strategies }) => {
      ...
      const results: ContractCallResults = await multicall.call(vaultCalls.concat(stratCalls));
...
export const mapContractCalls = (result: ContractCallReturnContext ) => {
    ...
    result.callsReturnContext.forEach(({ methodName, returnValues }) => {
        if (returnValues && returnValues.length > 0) {
            if (typeof returnValues[0] === 'string' || typeof returnValues[0] === 'boolean') {
                mappedObj[methodName] = returnValues[0];
            } else if (get(returnValues[0], 'type') === 'BigNumber') {
                mappedObj[methodName] = BigNumber.from(returnValues[0]).toString();
            }
        }
    });
    return mappedObj;
...
const mapVaultData = (contractCallsResults: ContractCallResults, vaultMap: Map<string, VaultApi>): Vault[] => {
    const vaults: Vault[] = [];
    ...
    vaultMap.forEach((vault, key) => {
        ...
        let mappedVault:any = {
            ...
            icon,
            emergencyShutdown,
            managementFee: get(vault, 'fees.general.managementFee', 'unknown') as string,
            performanceFee: get(vault, 'fees.general.performanceFee', 'unknown') as string,
